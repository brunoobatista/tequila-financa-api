...
CREATE INDEX IF NOT EXISTS fk_status_carteira_idx ON public.Carteira (status ASC);
...
CREATE TABLE IF NOT EXISTS public.DespesaVariavel (
  ...
  ativo INTEGER NOT NULL DEFAULT 1,
  PRIMARY KEY (id),
  CONSTRAINT fk_usuario_despesavrl_id
    FOREIGN KEY (usuario_id)
    REFERENCES public.Usuario (id)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX IF NOT EXISTS fk_usuario_despesavrl_idx ON public.DespesaVariavel (usuario_id ASC);
...
CREATE INDEX IF NOT EXISTS fk_usuario_despesafx_idx ON public.DespesaFixa (usuario_id ASC);
